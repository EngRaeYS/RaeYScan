...
class RAEYSCAN:
    def __init__(self, target):
        ...
        self.discovered_ips = []
        self.found_urls = []  # Store found URLs for browser opening
...
{Colors.CYAN}    ║{Colors.RESET}    {Colors.YELLOW}[10]{Colors.RESET} Open Found URLs in Chrome                   {Colors.CYAN}║{Colors.RESET}
...
{Colors.CYAN}    ╚══════════════════════════════════════════════════════════════════╝{Colors.RESET}
    ...
    def _check(self, platform, url):
        try:
            ...
            if r.status_code == 200:
                ...
                self.output["hits"].append(hit_data)
                self.found_urls.append(url)  # Store URL for browser opening
                return True
            else:
    ...
    def _open_in_chrome(self, urls):
        """Open URLs in Chrome browser"""
        if not urls:
            print(f"{Colors.YELLOW}[!] No URLs found to open.{Colors.RESET}")
            return
        
        print(f"\n{Colors.CYAN}[RAEYSCAN]{Colors.RESET} Opening {len(urls)} URLs in Chrome...")
        
        # Try multiple methods to open Chrome
        for url in urls:
            try:
                # Method 1: Try using webbrowser module with Chrome
                chrome_paths = [
                    "chrome",
                    "google-chrome",
                    "chromium",
                ...
                print(f"{Colors.GREEN}[✓]{Colors.RESET} Opened: {url[:60]}...")
                time.sleep(0.3)  # Small delay to prevent overwhelming the browser
                
            except Exception as e:
                print(f"{Colors.RED}[✗]{Colors.RESET} Failed to open: {url[:60]}...")
        
        print(f"{Colors.GREEN}[✓]{Colors.RESET} Finished opening URLs in Chrome.")
    ...
    def scan(self, mode=1):
        ...
        # Handle mode 10 - Open found URLs
        if mode == 10:
            self._open_in_chrome(self.found_urls)
            return self.output
        ...
        if mode not in [5, 6, 7, 9]:
            ...
            print(f"{Colors.CYAN}[i]{Colors.RESET} Found URLs stored. Use option {Colors.YELLOW}10{Colors.RESET} to open them in Chrome.")
...
<head>
    ...
    <style>
        ...
        .url-link {{ color: #00ff00; text-decoration: none; }}
        .url-link:hover {{ text-decoration: underline; color: #00ffff; }}
    </style>
</head>
<body>
    <div class="header">
                ...
                for key, val in hit.items():
                    if key != "platform":
                        if key == "url" and isinstance(val, str):
                            f.write(f'<div><a href="{val}" target="_blank" class="url-link">{key}: {val}</a></div>')
                        elif isinstance(val, list):
                            f.write(f"<div>{key}:</div><ul>")
                            for item in val:
                                if isinstance(item, str) and (item.startswith("http://") or item.startswith("https://")):
                                    f.write(f'<li><a href="{item}" target="_blank" class="url-link">{item}</a></li>')
                                else:
    ...
    if len(sys.argv) > 1:
        ...
        try:
            ...
            if choice == 0:
                ...
                sys.exit(0)
            scanner.scan(choice)
            if choice != 10:  # Don't save if just opening URLs
                scanner.save()
                scanner.export_html()
        except ValueError:
            print(f"{Colors.RED}[!] Invalid input. Running full scan.{Colors.RESET}")
